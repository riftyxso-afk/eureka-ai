...
> **Catatan implementasi (2026-09-04):** verifikasi live menunjukkan Juan Router belum menyediakan endpoint `/embeddings` dan `/audio/transcriptions` (semua 22 model hanya bertipe chat). Migrasi RAG ditahan — embedding/transkripsi tetap memakai provider yang dikonfigurasi via `AI_PROVIDER` (default OpenAgentic) sampai Juan mendukung endpoint tersebut. Larangan OpenAgentic untuk generate teks tetap berlaku penuh.
