...
export default function QuizModal({
  ...
  return (
    ...
    >
      <div
        className="card-clay max-h-[85vh] w-full max-w-lg overflow-y-auto !p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-extrabold text-clay-dark">Kuis AI</h2>
          <button
            onClick={onClose}
            aria-label="Tutup"
              ...
              <div className="flex gap-2">
                {[5, 8, 10].map((n) => (
                  <button
                    key={n}
                    onClick={() => setCount(n)}
                    className={`btn-clay-ghost flex-1 !min-h-[40px] text-sm ${
                      count === n
        ...
        {questions && (
          <div className="space-y-5">
            ...
            {questions.map((q, qi) => {
              ...
              return (
                <div key={q.id} className="rounded-2xl border-2 border-clay-shadow/40 bg-white/60 p-4">
                  <div className="mb-2 text-sm font-extrabold text-clay-dark">
                    {qi + 1}. {q.question}
                  </div>
                    ...
                    {q.options.map((opt, oi) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-center gap-2 rounded-xl border-2 bg-white px-3 py-2 text-left text-sm font-semibold text-clay-dark transition-colors disabled:cursor-default ${cls}`}
                        >
                          <span
                  ...
                  {submitted && q.explanation && (
                    <p className="mt-2 rounded-xl bg-clay-beige px-3 py-2 text-xs font-medium text-clay-dark">
                      💡 {q.explanation}
                    </p>
                  )}
                </div>
